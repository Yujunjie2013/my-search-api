...
public class ElasticSearchServiceImpl implements ElasticSearchService {
    ...
    public List<BookSearchResultVo> queryDocumentByParam(String indices, String type, BookSearchParam param) {
        ...
        //比如你分页10条 查第10页 会把前10也全部拿出来放到内存中 给你取你要的后面的数据. 不要查的太多 防止被人参数注入 内存爆炸 最多不要超过1000页.
    ...
    @Override
    public List<BookSearchResultVo> search(String keyWord) {
        try {
            return LuceneUtils.search("discription", keyWord, 1000, BookSearchResultVo.class);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
